module calibrate4NM
	...
	calibration=optimize(momentsgap4, [log(0.97/0.03), log(0.7/0.3), log(0.2/0.8), log(31.0/19.0)], NelderMead(), OptimizationOptions(show_trace=true) )
	
	showcompact(Optim.minimizer(calibration))
	bbetastar=Optim.minimizer(calibration)[1]
	bbetastar=exp(bbetastar)/(1+exp(bbetastar))
	defcost1star=Optim.minimizer(calibration)[2]
	defcost1star=-exp(defcost1star)/(1+exp(defcost1star))
	df2ratiostar=Optim.minimizer(calibration)[3]
	df2ratiostar=1+exp(df2ratiostar)/(1+exp(df2ratiostar))
	frequencystar=Optim.minimizer(calibration)[4]
	frequencystar=1+50*exp(frequencystar)/(1+exp(frequencystar))
	println(" \n Star parameters:")
	showcompact([bbetastar, defcost1star, df2ratiostar, frequencystar])
	...
	stareconparams=EconParams(
		...
		frequencystar, 	# panicfrequency::Float64 -- One every 16 quarters
