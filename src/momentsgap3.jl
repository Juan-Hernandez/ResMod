function momentsgap3(params::Array{Float64,1})
	# 0. Unpack
	bbeta=0.99*exp(params[1])/(1+exp(params[1]))
	defcost1=-0.1-0.95*exp(params[2])/(1+exp(params[2]))
	defcost2ratio=1+0.5*exp(params[3])/(1+exp(params[3]))
	...
	gapnorm=sum(errorvec)+max( 0.0, log(10, max(valuegap,pricegap,defaultgap)/solverparams.valtol/10.0) )
	return gapnorm
end
