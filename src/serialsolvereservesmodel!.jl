function serialsolvereservesmodel!(model::ReservesModel, solverparams=SolverParams())
	...
	bondcashflow=Array{Float64}(debtnum, resnum, mnum, ynum, regimenum)
	newvaluedefault=Array{Float64}(resnum, ynum, regimenum)
	newvaluepay=Array{Float64}(debtnum, resnum, mnum, ynum, regimenum)
	# Also need to allocate policies in  arrays
	debtpolicy=Array{Int64}(debtnum, resnum, mnum, ynum, regimenum)
	reservespolicy=Array{Int64}(debtnum, resnum, mnum, ynum, regimenum)
	defaultpolicy=Array{Bool}(debtnum, resnum, mnum, ynum, regimenum)
...
""" For prototyping we use just one evaluation, pmap below"""		
		...
		# pmap requires  arrays for inplace! outputs
