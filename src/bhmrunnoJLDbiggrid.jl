...
curriter=0
sizegrid=140
betagrid=collect(linspace(0.9816, 0.9864, 7))
def1grid=collect(linspace(-0.5, -0.7, 5))
def2grid=collect(linspace(0.92, 0.98, 4))
...
for i=1:sizegrid
	...
	baseeconparams=EconParams(
		# Consumer
		betagrid[mod1(i,7)],	# bbeta::Float64
		...
		def1grid[mod1(i,5)],# defcost1::Float64
		(0.88-(def1grid[mod1(i,5)]+0.7)/0.88)*def2grid[mod1(i,4)], 	# defcost2::Float64
		0.083, # reentry::Float64
		# Sudden Stop Probability
		40.0, 	# panicfrequency::Float64 -- One every 16 quarters
		4.0   # panicduration::Float64 -- 8 quarters
		)
