...
# 1. Define parameters
growth=0.0		# Quarterly growth rate
pivot=0.974 	# Pivot point for default cost

# 1.1 Sobol points over calibrated parameter box 

# Vector [beta, par1, par2, 1/freq]
calsequence=SobolSeq(4, [0.985, -0.01, 0.1, 0.025 ], [0.99, 0.07, 1.0, 0.0625])
...
pmap( serialmomentsimulator!, repeated(basecompuparams,itermax), 
	[ baseeconparams=EconParams(
		...
		0.0085+(0.01-growth)/(1+growth),# coupon:: Float64 # 340bp divided by 4 plus r (spread is the same with growth, r is not)
		0.843, 							# logOutputRho::Float64
		0.0223, 						# logOutputSigma::Float64
