function solvereservesmodelserial!(model::ReservesModel, solverparams=SolverParams)
	...
	newvaluepay=Array{Float64}( debtnum, resnum, model.compuparams.mnum, ynum, regimenum)
	# Also need to allocate policies in shared arrays
	debtpolicy=Array{Int64}(debtnum, resnum, model.compuparams.mnum, ynum, regimenum)
	reservespolicy=Array{Int64}( debtnum, resnum, model.compuparams.mnum, ynum, regimenum)
	defaultpolicy=Array{Bool}( debtnum, resnum, model.compuparams.mnum, ynum, regimenum)
...
""" For prototyping we use just one evaluation, pmap below"""		
		...
		map(updatevaluepaydrm!, [ sub(newvaluepay, :, :, :, iyr) for iyr=1:exonum], [sub(newbondprice, :, :, iyr) for iyr=1:exonum], # Outputs
