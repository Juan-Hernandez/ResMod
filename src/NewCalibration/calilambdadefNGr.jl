...
pmap( momentsimulator!, Iterators.repeated(basecompuparams,itermax), 
	[ baseeconparams=EconParams(
		# Consumer
		parvec[1]*(1.0+growth)^(1-ggamma), # bbeta*(1.0+growth)^(1-ggamma),			# bbeta::Float64
		...
		(1.0/parvec[2]+growth)/(1.0+growth), 				# llambda::Float64 		# 5 year avg maturity 
		coupon-growth*(1.0-1.0/parvec[2])/(1.0+growth), 	# coupon:: Float64 
		...
		20.0, 										# panicfrequency::Float64 -- One every 16 quarters
